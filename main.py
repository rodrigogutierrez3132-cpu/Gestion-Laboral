...
def firmar_documento(doc_id: int, rut_trabajador: str = Form(...), request: Request = None):
    ...
    cursor = conn.cursor()
    try:
        # Validación para evitar duplicados si ya cuenta con una firma registrada
        cursor.execute("SELECT * FROM firmas_documentos WHERE documento_id = ?", (doc_id,))
        firmado_previo = cursor.fetchone()
        if firmado_previo:
            cursor.execute("SELECT * FROM usuarios WHERE rut = ?", (rut_trabajador,))
            user = cursor.fetchone()
            return render_worker_dashboard(user, "Este documento ya cuenta con una firma registrada.")
